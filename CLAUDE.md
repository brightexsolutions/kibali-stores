...
- **Fixed real bug (2026-07-04): `@next/swc-darwin-arm64` must NEVER be a committed dependency.** It was pinned as a regular `devDependency` back at M0 because this dev sandbox's npm skipped installing optional platform binaries, and `next build`/`next dev` needs the native SWC binary. That pin broke the real Vercel deploy (`EBADPLATFORM` — Vercel builds on Linux x64, the package is Darwin arm64-only, and a plain devDependency installs unconditionally regardless of platform, unlike npm's own optionalDependencies mechanism which correctly skips incompatible platforms). Removed from `apps/web/package.json` and `package-lock.json` regenerated. If this sandbox needs the binary again for a local `next build`/`next dev`, install it with `npm install --no-save @next/swc-darwin-arm64` — never `--save`/plain `npm install <pkg>`, and never commit the result. `tsc --noEmit` doesn't need this binary at all, so prefer that for local type-checking.
